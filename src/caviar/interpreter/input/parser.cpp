...
#define PROCESS_VAL(p) \
  case (p):            \
    ...
    switch (value)
    {
      PROCESS_VAL(Kind::identifier);
      PROCESS_VAL(Kind::string);
      PROCESS_VAL(Kind::int_number);
      PROCESS_VAL(Kind::real_number);
      PROCESS_VAL(Kind::eof);
      PROCESS_VAL(Kind::plus);
      PROCESS_VAL(Kind::minus);
      PROCESS_VAL(Kind::mul);
      PROCESS_VAL(Kind::div);
      PROCESS_VAL(Kind::truediv);
      PROCESS_VAL(Kind::modulus);
      PROCESS_VAL(Kind::assign);
      PROCESS_VAL(Kind::asplus);
      PROCESS_VAL(Kind::asminus);
      PROCESS_VAL(Kind::asmul);
      PROCESS_VAL(Kind::asdiv);
      PROCESS_VAL(Kind::lp);
      PROCESS_VAL(Kind::rp);
      PROCESS_VAL(Kind::squote);
      PROCESS_VAL(Kind::dquote);
      PROCESS_VAL(Kind::comment);
      PROCESS_VAL(Kind::backslash);
      PROCESS_VAL(Kind::eol);
      PROCESS_VAL(Kind::lcb);
      PROCESS_VAL(Kind::rcb);
      PROCESS_VAL(Kind::lsb);
      PROCESS_VAL(Kind::rsb);
      PROCESS_VAL(Kind::eqsmaller);
      PROCESS_VAL(Kind::eqlarger);
      PROCESS_VAL(Kind::equal);
      PROCESS_VAL(Kind::smaller);
      PROCESS_VAL(Kind::larger);
...
#undef PROCESS_VAL
  ...
  {
    auto token = get_raw_token();
    if (token.kind == caviar::interpreter::Kind::string)
      stream << token.string_value << std::endl;
    else if (token.kind == caviar::interpreter::Kind::identifier)
    {
      ...
      stream << expression(false) << std::endl;
    return stream;
  }
  ...
  std::string Parser::get_identifier()
  {
    ...
    if (token.kind == caviar::interpreter::Kind::identifier)
      ...
      error->all(FC_FILE_LINE_FUNC_LINE_COL, "Expected identifier");
    return ""; //-Wreturn-type
  }
  ...
  bool Parser::compare_real()
  {
    ...
    while (true)
    {
      ...
      if (t.kind == caviar::interpreter::Kind::eol)
        break;
      else if (t.kind == caviar::interpreter::Kind::smaller)
      { //-2
        ++num_of_condition_operators;
        operator_called = true;
        condition_operator = -2;
      }
      else if (t.kind == caviar::interpreter::Kind::larger)
      { //+2
        ++num_of_condition_operators;
        operator_called = true;
        condition_operator = +2;
      }
      else if (t.kind == caviar::interpreter::Kind::eqsmaller)
      { //-1
        ++num_of_condition_operators;
        operator_called = true;
        condition_operator = -1;
      }
      else if (t.kind == caviar::interpreter::Kind::eqlarger)
      { //+1
        ++num_of_condition_operators;
        operator_called = true;
        condition_operator = +1;
      }
      else if (t.kind == caviar::interpreter::Kind::equal)
      { // 0
        ++num_of_condition_operators;
        operator_called = true;
    ...
    ///*
    if (condition_operator == -2)
      return (left_side < right_side);
    if (condition_operator == -1)
      return (left_side <= right_side);
    if (condition_operator == 0)
      return (left_side == right_side);
    if (condition_operator == 1)
      return (left_side >= right_side);
    if (condition_operator == 2)
  ...
  int Parser::get_positive_int()
  {
    int i = get_int();
    if (i < 0)
      error->all(FC_FILE_LINE_FUNC_LINE_COL, "expected a positive integer value.");
    return int(expression(true));
  }
  ...
  */
  ...
  int Parser::get_literal_int()
  {
    ...
    if (token.kind == caviar::interpreter::Kind::int_number)
      ...
      error->all(FC_FILE_LINE_FUNC_LINE_COL, "Expected integer literal");
    return 0; //-Wreturn-type
  }
  ...
  std::string Parser::get_literal_string()
  {
    ...
    if (token.kind == caviar::interpreter::Kind::string)
      ...
      error->all(FC_FILE_LINE_FUNC_LINE_COL, "Expected string literal");
    return ""; //-Wreturn-type
  }
  ...
  double Parser::primary(bool get)
  {
    ...
    case Kind::lp:
    {
      ...
      if (token_stream->current().kind != Kind::rp)
        error->all(FC_FILE_LINE_FUNC_LINE_COL, "')' expected");
      get_raw_token();
      return e;
    }
