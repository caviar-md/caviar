...
namespace interpreter
{
  ...
  Token Token_stream::get()
  {
    ...
    while (ch == '\\')
    {
      if (get_char() == '\n')
      {
        ...
        error->all(FC_FILE_LINE_FUNC_LINE_COL, "unexpected character after line continuation character");
    }
  ...
  Token Token_stream::get_number_literal(char first_char)
  {
    ...
    if (num_is_real)
      line_stream >> ct.real_value;
    else
      line_stream >> ct.int_value;
    return ct;
  }
    ...
    while (line_stream.get(ch))
    {
      ...
      else if (ch == '\n')
      {
        if (triple)
        {
          ...
          error->all(FC_FILE_LINE_FUNC_LINE_COL, "EOL while scanning string literal");
      }
      else if (ch == eof)
        error->all(FC_FILE_LINE_FUNC_LINE_COL, "EOF while scanning triple-quoted string literal");
      else
        ct.string_value += ch;
    }
