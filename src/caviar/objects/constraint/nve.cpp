...
bool Nve::read (caviar::interpreter::Parser *parser) {
  ...
  while(true) {
    ...
    if (string_cmp(t,"energy_per_dof")) {
      ...
      if (energy_per_dof <= 0.0) error->all (FC_FILE_LINE_FUNC_PARSE, "energy_per_dof have to non-negative."); 
    } else if (string_cmp(t,"energy_tot")) {
      GET_OR_CHOOSE_A_REAL(energy_tot,"","")
      if (energy_tot <= 0.0) error->all (FC_FILE_LINE_FUNC_PARSE, "energy_tot have to non-negative."); 
      ...
      if (kbt <= 0.0) error->all (FC_FILE_LINE_FUNC_PARSE, "kbt have to non-negative."); 
