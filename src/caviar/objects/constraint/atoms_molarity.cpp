...
namespace constraint
{
  ...
  Atoms_molarity::Atoms_molarity(CAVIAR *fptr) : Constraint{fptr}
  {
    ...
    minimum_set = false;
    maximum_set = false;
    constraint_type = Constraint_t::Atoms_molarity;
  }

  Atoms_molarity::~Atoms_molarity() {}

  bool Atoms_molarity::read(caviar::interpreter::Parser *parser)
  {
    FC_OBJECT_READ_INFO
    bool in_file = true;
    while (true)
    {
      ...
      if (string_cmp(t, "add_atom_type"))
      {
        ...
        if (i < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "atom_type have to be non-negative.");
      ...
      else if (string_cmp(t, "check_steps"))
      {
        ...
        if (check_steps < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "check_steps have to be non-negative.");
      }
      else if (string_cmp(t, "creation_try"))
      {
        GET_OR_CHOOSE_A_INT(creation_try, "", "")
        if (creation_try < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "creation_try have to be non-negative.");
      }
      else if (string_cmp(t, "minimum_limit"))
      {
        GET_OR_CHOOSE_A_INT(minimum_limit, "", "")
        if (minimum_limit < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "minimum_limit have to be non-negative.");
        minimum_set = true;
      }
      else if (string_cmp(t, "maximum_limit"))
      {
        ...
        if (maximum_limit < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "maximum_limit have to be non-negative.");
        maximum_set = true;
      }
  ...
  void Atoms_molarity::apply(int64_t steps)
  { // III
