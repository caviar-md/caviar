...
bool Fene_bond::read (caviar::interpreter::Parser *parser) {
  ...
  while(true) {
    ...
    if (string_cmp(t,"elastic_coef")) {
      ...
      if (vector_value < 0)  error->all (FC_FILE_LINE_FUNC_PARSE, "Epsilon coef. have to be non-negative.");            
    }  else if (string_cmp(t,"sigma_coef")) {
      GET_A_STDVECTOR_REAL_ELEMENT(sigma_coef)
      if (vector_value < 0)  error->all (FC_FILE_LINE_FUNC_PARSE, "Sigma coef. have to be non-negative.");            
    }  else if (string_cmp(t,"R_0")) {
      GET_A_STDVECTOR_REAL_ELEMENT(R)
      if (vector_value < 0)  error->all (FC_FILE_LINE_FUNC_PARSE, "R_0 have to be non-negative.");                        
    }  else if (string_cmp(t,"repulsive_cutoff")) {
      GET_OR_CHOOSE_A_REAL(repulsive_cutoff,"","")
      if (repulsive_cutoff < 0.0) error->all (FC_FILE_LINE_FUNC_PARSE, "Repulsive cutoff have to non-negative.");                          
