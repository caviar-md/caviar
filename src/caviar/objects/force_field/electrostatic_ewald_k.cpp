...
namespace force_field
{
  ...
  Electrostatic_ewald_k::Electrostatic_ewald_k(CAVIAR *fptr) : Force_field{fptr},
                                                               k_electrostatic{1.0}
  {
  ...
  bool Electrostatic_ewald_k::read(caviar::interpreter::Parser *parser)
  {
    FC_OBJECT_READ_INFO
    bool in_file = true;
    ...
    while (true)
    {
      ...
      else if (string_cmp(t, "kz_max"))
      {
        ...
        if (kz_max < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "'kz_max' have to non-negative.");
      }
      ...
      else if (string_cmp(t, "epsilon_dipole"))
      {
        ...
        if (epsilon_dipole < 0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "'epsilon_dipole' has to be non-negative.");
        dipole = true;
      }
      else if (string_cmp(t, "slab_lz") || string_cmp(t, "slab_thickness"))
      {
        ...
        if (slab_lz < 0.0)
          error->all(FC_FILE_LINE_FUNC_PARSE, "slab_lz have to non-negative.");
      }
  ...
  void Electrostatic_ewald_k::verify_settings()
  {
