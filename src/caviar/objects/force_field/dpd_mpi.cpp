...
bool Dpd_mpi::read (caviar::interpreter::Parser *parser) {
  ...
  while(true) {
    ...
    if (string_cmp(t,"cutoff")) {
      ...
      if (rnd_seed < 0.0) error->all (FC_FILE_LINE_FUNC_PARSE, "random_seed have to non-negative.");      
      rnd_generator.seed(rnd_seed);      
    } else if (string_cmp(t,"conserv_coef")) {
      GET_A_STDVECTOR_STDVECTOR_REAL_ELEMENT(conserv_coef)
      if (vector_value < 0)  error->all (FC_FILE_LINE_FUNC_PARSE, "conserv_coef have to be non-negative.");      
    }  else if (string_cmp(t,"dissip_coef")) {
      GET_A_STDVECTOR_STDVECTOR_REAL_ELEMENT(dissip_coef)
      if (vector_value < 0)  error->all (FC_FILE_LINE_FUNC_PARSE, "dissip_coef have to be non-negative.");            
...
void Dpd_mpi::calculate_acceleration () {
  ...
  for (unsigned i=0; i < domain -> neighborlist_domains.size(); ++i) { // this doesn't need to be reconstructed in every step
  ...
  for (unsigned i=0; i < atom_data -> owned.id.size(); ++i) { // this doesn't need to be reconstructed in every step
