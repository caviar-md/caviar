...
Lj::Lj (CAVIAR *fptr) : Force_field{fptr} {
  ...
  jump_fix = false; monitor_jump = false; jump_tol = 1e-6; 
  wca = false;
...
bool Lj::read (caviar::interpreter::Parser *) {
  /*
  FC_OBJECT_READ_INFO
  bool in_file = true;
...
void Lj::verify_settings() {
  ...
  if (input_by_atom)  {
    ...
    for (auto && e : epsilon)  e.resize(atom_max_size,0);
  
  
    for (auto && s : sigma) s.resize(atom_max_size,0);
  ...
  if (wca)  {
    ...
    for (auto && c : cutoff_list) c.resize(max_size,0);    
...
void Lj::calculate_acceleration () {
  ...
  for (unsigned int i=0; i<nlist.size (); ++i) {
    ...
    for (auto j : nlist[i]) {
      ...
      if (cutoff_list_activated) c = cutoff_list[type_i][type_j]* cutoff_list[type_i][type_j];
      ...
      if (jump_fix) {
        ...
        if (monitor_jump) 
          if (dr_sq < jump_tol) std::cout << "\ndr_sq: " << dr_sq << " ";    
        
        if (dr_sq < jump_tol) dr_sq = jump_tol;
      }

      const auto eps_ij = epsilon [type_i] [type_j];
      if (eps_ij==0) continue; // used in water simulations. Hydrogen in water models usiually has zero value.
