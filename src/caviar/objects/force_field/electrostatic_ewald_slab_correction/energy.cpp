...
#pragma omp parallel for reduction(+ \
    ...
    for (auto ip = 0; ip < kx_max * ky_max; ++ip)
    {
      ...
      const double sum_chi =
          -FC_POW2(chi_p[ip][0][0][0]) - FC_POW2(chi_p[ip][0][0][1]) - FC_POW2(chi_p[ip][0][1][0]) + FC_POW2(chi_p[ip][1][0][0]) - FC_POW2(chi_p[ip][0][1][1]) + FC_POW2(chi_p[ip][1][0][1]) + FC_POW2(chi_p[ip][1][1][0]) + FC_POW2(chi_p[ip][1][1][1]);
...
#pragma omp parallel for reduction(+ \
    ...
    for (auto ix = 0; ix < kx_max; ++ix)
    {
      ...
      const double sum_chi =
          -FC_POW2(chi_x[ix][0][0]) - FC_POW2(chi_x[ix][0][1]) + FC_POW2(chi_x[ix][1][0]) + FC_POW2(chi_x[ix][1][1]);
...
#pragma omp parallel for reduction(+ \
    ...
    for (auto iy = 0; iy < ky_max; ++iy)
    {
      ...
      const double sum_chi =
          -FC_POW2(chi_y[iy][0][0]) - FC_POW2(chi_y[iy][0][1]) + FC_POW2(chi_y[iy][1][0]) + FC_POW2(chi_y[iy][1][1]);
