...
bool Electrostatic_ewald_slab_correction::read (caviar::interpreter::Parser *parser) {
  ...
  while(true) {
    ...
    if (string_cmp(t,"k_max")) {
      ...
      if (lz < 0)  error->all (FC_FILE_LINE_FUNC_PARSE, "'lz' has to be non-negative.");            
      ...
      } else error->all (FC_FILE_LINE_FUNC_PARSE, "expected 'x', 'y' or 'z'.");      
      if (slab_normal_axis == 0) output->info("slab local axis 'x' = simulation "
          "global 'y' axis, slab local axis 'y' = simulation global 'z' axis." ); 

      if (slab_normal_axis == 1) output->info("slab local axis 'x' = simulation "
          "global 'x' axis, slab local axis 'y' = simulation global 'z' axis." ); 

      if (slab_normal_axis == 2) output->info("slab local axis 'x' = simulation "
