...
bool Verlet_list::read (caviar::interpreter::Parser *parser) {
  ...
  while(true) {
    ...
    if (string_cmp(t,"set_atom_data") || string_cmp(t,"atom_data")) {
      ...
      if (cutoff_extra_coef < 0.0) error->all (FC_FILE_LINE_FUNC_PARSE, "cutoff_extra_coef have to non-negative."); 
...
void Verlet_list::init () {
  ...
  if (cutoff <= 0.0) error->all (FC_FILE_LINE_FUNC, "cutoff have to non-negative."); 
  if (dt <= 0.0) error->all (FC_FILE_LINE_FUNC, "dt have to non-negative.");
