...
namespace shape
{
  namespace polyhedron
  {
    ...
    void Utility::make_normal(shape::polyhedron::Polyhedron &p_object)
    { // It's supposed here that the vertices are written in order (right-hand rotation)
      const auto &vertex = p_object.vertex;
      const auto &face = p_object.face;
      auto &normal = p_object.normal;
