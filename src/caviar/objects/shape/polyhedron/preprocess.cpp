...
namespace shape
{
  namespace polyhedron
  {
    ...
    void Preprocess::pre_correct_normals(shape::polyhedron::Polyhedron &p_object)
    {
      ...
      if (face.size() == 0 || edges.size() == 0)
        error->all(FC_FILE_LINE_FUNC, "The polyhedron face or edge is empty");
...
#undef MAKE_EDGE_LIST
      ...
      for (unsigned int i = 0; i < edge_list.size(); ++i)
      {
        ...
        if (it == edges.end())
        {
          error->all(FC_FILE_LINE_FUNC, "a bug in the code 1!");
        } //
        if (it->second.size() != 2)
        {
          error->all(FC_FILE_LINE_FUNC, "a bug in the code 2!");
        } //

        {
          ...
          if (f1_right == f2_right)
          { // if two neighborlist faces has similar twist order in one edge,
            // their normals will be directed differently. so it has to be corrected.
            if (f2 == face_out[i])
            { // only change the outer face
