...
//    if (gxhi < xhi) gxhi = xhi;    if (gyhi < yhi) gyhi = yhi;    if (gzhi < zlo) gzhi = zhi;
  ...
  std::cout << " min. and max. of Preprocess shape " << " ? " << " coordinates:"
            << " xlo: " << xlo << " xhi: "   << xhi \
            << " ylo: " << ylo << " yhi: "   << yhi \
...
void Postprocess::make_grid (shape::polyhedron::Polyhedron & p_object) {
  ...
  for (unsigned int i=0;i<face.size();++i) {
    ...
    fxlo -= tol; fylo -= tol; fzlo -= tol;
    fxhi += tol; fyhi += tol; fzhi += tol;    
    ...
    if (xindex_hi>int_floor(nx_part-1)) xindex_hi = nx_part-1;
    if (yindex_hi>int_floor(ny_part-1)) yindex_hi = ny_part-1;
    if (zindex_hi>int_floor(nz_part-1)) zindex_hi = nz_part-1;
