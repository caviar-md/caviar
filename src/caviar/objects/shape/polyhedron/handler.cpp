...
namespace shape
{
  namespace polyhedron
  {
      ...
      while (true)
      {
        ...
        else if (string_cmp(t, "grid"))
        {
          ...
          if (nx_ < 1 || ny_ < 1 || nz_ < 1)
            error->all(FC_FILE_LINE_FUNC_PARSE, "grids has to be larger than 1");
          polyhedron.nx_part = nx_;
          polyhedron.nx_part = ny_;
          polyhedron.nx_part = nz_;
        }
        else
    ...
    void Handler::command_generate()
    {
      ...
      if (output_mesh_povray)
        polyhedron_output->mesh_povray(polyhedron);
      if (output_mesh_tcl)
        polyhedron_output->mesh_tcl(polyhedron);
      if (output_normals_tcl)
        polyhedron_output->normals_tcl(polyhedron);
      if (output_normals_vectors)
        polyhedron_output->normals_vectors(polyhedron);
      if (output_edges_tcl)
        polyhedron_output->edges_tcl(polyhedron);
    }
