...
namespace caviar
{
  ...
  namespace shape
  {
    namespace polyhedron
    {
      ...
      Point_Inside::Point_Inside(CAVIAR *fptr) : caviar_{fptr},
                                   comm{fptr->comm},
                                   error{fptr->error},
                                   output{fptr->output},
                                   input{fptr->input},
      ...
      bool Point_Inside::is_inside_grid(shape::polyhedron::Polyhedron &p_object, const Vector<double> &v, const double r)
      {
        if (!is_inside(p_object, v))
          return false;
        if (in_contact_grid(p_object, v, r))
          return false;
        return true;
      }
      ...
      bool Point_Inside::is_inside_all(shape::polyhedron::Polyhedron &p_object, const Vector<double> &v, const double r)
      {
        if (!is_inside(p_object, v))
          return false;
        if (in_contact_all(p_object, v, r))
          return false;
        return true;
      }
                                         ...
                                         const Vector<double> &v, const double radius, Vector<double> &contact_vector)
      {
        ...
        if (xindex > int_floor(nx_part - 1))
          return false;
        if (yindex > int_floor(ny_part - 1))
          return false;
        if (zindex > int_floor(nz_part - 1))
          return false;
...
#define VEC_GREATER_THAN_VERTEX(VAR, AXIS)                                                                            \
  if (VAR.AXIS > vertex[face[i][0]].AXIS && VAR.AXIS > vertex[face[i][1]].AXIS && VAR.AXIS > vertex[face[i][2]].AXIS) \
    continue;
