  ...
  namespace polyhedron
  {
    ...
    void Format_unv_reader::read_polyhedron(shape::polyhedron::Polyhedron &p_object, const std::string &filename)
    {
      ...
      while (!ifs.eof())
      {
        ...
        {
          if (udn_code == 2411)
            import_udn_2411(p_object, ifs);
          else if (udn_code == 2412)
            import_udn_2412(p_object, ifs);
          else if (udn_code == 2467)
            import_udn_2467(p_object, ifs);
          else
...
#undef ADD_EDGE
    ...
    void Format_unv_reader::make_label_to_index(const std::vector<int> &u, std::vector<int> &v)
    {
