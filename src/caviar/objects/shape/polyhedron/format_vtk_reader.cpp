...
void Format_vtk_reader::read_polyhedron (shape::polyhedron::Polyhedron &p_object, const std::string & file) {
  ...
  std::cout << "info: Vtk_file: recording vtk file as a 3D shape with index " << " ? " << std::endl;

  while(true) {

    auto t0 = parser->get_val_token ();
    struct caviar::interpreter::Token t1;
    if (t0.kind==caviar::interpreter::Kind::eof) break;
    ...
    if (t0.kind==caviar::interpreter::Kind::identifier) {
      if (t0.string_value == "POINTS") {
        ...
        while (true) {
          ...
          if (t1.kind==caviar::interpreter::Kind::identifier) break;
          if (t1.kind==caviar::interpreter::Kind::eol) t1 = parser->get_val_token ();
          if (t1.kind==caviar::interpreter::Kind::identifier) break;
          if (xyz==0) {
