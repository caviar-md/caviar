...
#include "caviar/objects/shape/plane.hpp"
...
namespace caviar
{
  ...
  namespace shape
  {
    ...
    bool Plane::on_the_plane(const Vector3d<double> &v)
    {
    ...
    bool Plane::is_inside(const Vector3d<double> &v)
    {
      if (!on_the_plane(v))
        return false;
      Vector3d<double> v_1 = v - center;
    ...
    bool Plane::is_inside(const Vector3d<double> &v, const double r)
    {
      if (!on_the_plane(v))
        return false;
      Vector3d<double> v_1 = v - center;
    ...
    bool Plane::in_contact(const Vector3d<double> &v, const double r, Vector3d<double> &contact_vector)
    {
