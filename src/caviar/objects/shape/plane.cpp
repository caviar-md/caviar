...
bool Plane::read (caviar::interpreter::Parser * parser) {
  ...
  while(true) {
    ...
    else ASSIGN_REAL_3D_VECTOR(normal,"Plane read: ","")
    else ASSIGN_REAL(radius,"Plane read:","")
    else ASSIGN_REAL(flatness_tol,"Plane read:","")
    else error->all (FC_FILE_LINE_FUNC_PARSE, " Plane read: Unknown variable or command");
  }
