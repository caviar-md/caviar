...
namespace caviar
{
  ...
  namespace shape
  {
    ...
    Circle::Circle(CAVIAR *fptr) : Shape{fptr}, flatness_tol{0.001} {
                                                    FC_OBJECT_INITIALIZE_INFO} Circle::~Circle() {}
    ...
    bool Circle::on_the_plane(const Vector3d<double> &v)
    {
    ...
    bool Circle::is_inside(const Vector3d<double> &v)
    {
      if (!on_the_plane(v))
        return false;
      Vector3d<double> v_1 = v - center;
    ...
    bool Circle::is_inside(const Vector3d<double> &v, const double r)
    {
      if (!on_the_plane(v))
        return false;
      Vector3d<double> v_1 = v - center;
    ...
    bool Circle::in_contact(const Vector3d<double> &v, const double r, Vector3d<double> &contact_vector)
    {
