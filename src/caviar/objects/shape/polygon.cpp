...
#include "caviar/objects/shape/polygon.hpp"
...
namespace caviar
{
  ...
  namespace shape
  {
    ...
    bool Polygon::is_inside(const Vector3d<double> &v)
    {
    ...
    bool Polygon::is_inside(const Vector3d<double> &v, const double r)
    {
      // error->all (FC_FILE_LINE_FUNC_PARSE, "not_implemented");
    ...
    bool Polygon::read(caviar::interpreter::Parser *parser)
    {
      FC_OBJECT_READ_INFO
      bool in_file = true;
      std::vector<Vector3d<double>> tmp;
      int no_points = 0;
      while (true)
      {
    ...
    void Polygon::make_basis_vectors()
    {
      if (p_3D.size() > 1)
      {
        Vector3d<double> vec1 = p_3D[1] - p_3D[0];
        Vector3d<double> vec2 = p_3D[2] - p_3D[0];
    ...
    void Polygon::make_uv_vectors()
    {
      ...
      {
        Vector3d<double> dp = p_3D[i] - p_3D[0];
    ...
    void Polygon::uv_to_xyz(double u_i, double v_i, Vector3d<double> &p_i)
    {
    ...
    void Polygon::xyz_to_uv(const Vector3d<double> &p_i, double &u_i, double &v_i)
    {
      Vector3d<double> dp = p_i - p_3D[0];
      u_i = u_vector * dp;
      v_i = v_vector * dp;
    }
    ...
    bool Polygon::in_contact(const Vector3d<double> &v, const double r, Vector3d<double> &contact_vector)
    {
