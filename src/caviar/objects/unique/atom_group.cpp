...
void Atom_group::add_atom(const objects::unique::Atom &a) {
  atoms.push_back(a);
}
...
Vector<double> Atom_group::pos_tot () const {
  if (part_of_a_atom_group) return position + upper_level_atom_group->pos_tot();
   else return position;   
}

Vector<double> Atom_group::vel_tot () const {
  if (part_of_a_atom_group) return velocity + upper_level_atom_group->vel_tot();
