...
namespace unique
{
  ...
  bool Random_1D::read(caviar::interpreter::Parser *parser)
  {
    ...
    while (true)
    {
      ...
      else ASSIGN_REAL(min, "Random_1D creation: ", "") else ASSIGN_REAL(max, "Random_1D creation: ", "") else ASSIGN_REAL(stddev, "Random_1D creation: ", "") else ASSIGN_REAL(mean, "Random_1D creation: ", "") else ASSIGN_INT(seed, "Random_1D creation: ", "") else error->all(FC_FILE_LINE_FUNC_PARSE, "Random_1D creation: Unknown variable or command ");
    }
    return in_file;
    ;
  }
  ...
  void Random_1D::generate()
  {
    ...
    else if (type == "NORMAL")
    {
      ...
      error->all(FC_FILE_LINE_FUNC, "Expected NORMAL or UNIFORM for the type. ");
  }
