  ...
  for (auto i=-1;i<2;++i)
    for (auto j=-1;j<2;++j)
      for (auto k=-1;k<2;++k) {
        ...
        if (i ==  0) ii = grid_index_x;
        if (i == -1) ii = grid_index_x == 0 ? nprocs_x-1 : (grid_index_x-1)%nprocs_x;
        if (i == +1) ii = grid_index_x == nprocs_x-1 ? 0 : (grid_index_x+1)%nprocs_x;
        if (j ==  0) jj = grid_index_y;
        if (j == -1) jj = grid_index_y == 0 ? nprocs_y-1 : (grid_index_y-1)%nprocs_y;
        if (j == +1) jj = grid_index_y == nprocs_y-1 ? 0 : (grid_index_y+1)%nprocs_y;
        if (k ==  0) kk = grid_index_z;
        if (k == -1) kk = grid_index_z == 0 ? nprocs_z-1 : (grid_index_z-1)%nprocs_z;
        if (k == +1) kk = grid_index_z == nprocs_z-1 ? 0 : (grid_index_z+1)%nprocs_z;
...
Vector<Real_t> Domain::periodic_distance (const Vector<Real_t> v) {
  ...
  if (boundary_condition.x == 1) {
    while (vf.x > +domain_dh.x ) {vf.x -= domain_dh.x*2.0;}
    while (vf.x < -domain_dh.x ) {vf.x += domain_dh.x*2.0;}
  }

  if (boundary_condition.y == 1) {
    while (vf.y > +domain_dh.y ) {vf.y -= domain_dh.y*2.0;}
    while (vf.y < -domain_dh.y ) {vf.y += domain_dh.y*2.0;}
  }

  if (boundary_condition.z == 1) {
    while (vf.z > +domain_dh.z ) {vf.z -= domain_dh.z*2.0;}
    while (vf.z < -domain_dh.z ) {vf.z += domain_dh.z*2.0;}
  }
