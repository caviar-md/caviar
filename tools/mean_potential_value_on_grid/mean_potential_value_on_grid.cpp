...
int main () {
  ...
  //for (int k = 0; k < 200; ++k)
  {
    ...
    std::cout << frame << " " ;
    if (ifs.eof()) break; // break if eof is reached
    ...
    if (in_first_frame)
    {
      if (max_grid_index_x < x_index)  max_grid_index_x = x_index;
      if (max_grid_index_y < y_index)  max_grid_index_y = y_index;
      if (max_grid_index_z < z_index)  max_grid_index_z = z_index;
      
      if (x_vector.size() < x_index)  x_vector.resize(x_index, 0.0);
      if (y_vector.size() < y_index)  y_vector.resize(y_index, 0.0);
      if (z_vector.size() < z_index)  z_vector.resize(z_index, 0.0);      
      
      
      switch (profile_direction)
      {
        case 'x': if (sum_potential.size() < x_index)  sum_potential.resize(x_index, 0.0); break;
        case 'y': if (sum_potential.size() < y_index)  sum_potential.resize(y_index, 0.0); break;
        case 'z': if (sum_potential.size() < z_index)  sum_potential.resize(z_index, 0.0); break;
        default: break;
      }
      ...
      if (set_first_index_in_cross_section)
      {
        switch (profile_direction)
        {
          case 'x': first_index_in_cross_section = x_index; break;
          case 'y': first_index_in_cross_section = y_index; break;
          case 'z': first_index_in_cross_section = z_index; break;
          default: break;
        }
      ...
      else
      {
        switch (profile_direction)
        {
          case 'x': if (first_index_in_cross_section == x_index) num_of_points_in_one_cross_section++ ; break;
          case 'y': if (first_index_in_cross_section == y_index) num_of_points_in_one_cross_section++ ; break;
          case 'z': if (first_index_in_cross_section == z_index) num_of_points_in_one_cross_section++ ; break;
          default: break;
        }
      }
    }
    ...
    switch (profile_direction)
    {
      case 'x': sum_potential[x_index] += tmp; break;
      case 'y': sum_potential[y_index] += tmp; break;
      case 'z': sum_potential[z_index] += tmp; break;
      default: break;
    }              
  ...
  switch (profile_direction)
  {
    case 'x': sum_potential.resize(max_grid_index_x, 0.0); break;
    case 'y': sum_potential.resize(max_grid_index_y, 0.0); break;
    case 'z': sum_potential.resize(max_grid_index_z, 0.0); break;
    default: break;
  }
  ...
  for (int i = 0; i < sum_potential.size(); ++i)
  {
    ...
    switch (profile_direction)
    {
      case 'x': ofs << x_vector[i] << " " << sum_potential[i]/total_num_of_points_in_one_cross_section << "\n"; break;
      case 'y': ofs << y_vector[i] << " " << sum_potential[i]/total_num_of_points_in_one_cross_section << "\n"; break;
      case 'z': ofs << z_vector[i] << " " << sum_potential[i]/total_num_of_points_in_one_cross_section << "\n"; break;
      default: break;
    }
