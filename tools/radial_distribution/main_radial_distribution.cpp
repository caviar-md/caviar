...
int main(int argc, char **argv)
{
  ...
  while (!ifs.eof())
  {
    ...
    for (int j = 0; j < num_of_atoms; ++j)
    {
      ...
      if (small_box_min.x < position[j].x && position[j].x < small_box_max.x && small_box_min.y < position[j].y && position[j].y < small_box_max.y && small_box_min.z < position[j].z && position[j].z < small_box_max.z)
      {
    ...
    ++frame_counter;
    if (frame_counter < start_xyz_frame)
      continue;
