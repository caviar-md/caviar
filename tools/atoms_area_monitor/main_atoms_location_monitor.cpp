...
{
  if (type + 1 > atom_type_to_id.size())
    return -1;
...
int main(int argc, char **argv)
{
  ...
  //=======================================================================
  if (debug)
    std::cout << "---------------- Set area intervals to be monitored -----------------\n";
  //=======================================================================
  ...
  */
  ...
  for (auto &&c : transition_counter)
    c.init(num_areas + 1);

  std::vector<AreaMatrix> transition_event_counter(num_atom_types);
  for (auto &&c : transition_event_counter)
    c.init(num_areas + 1);
  ...
  while (!ifs.eof())
  {
    ...
    if (debug_read)
      std::cout << "dummyLine 1: " << dummyLine << "\n";
    std::getline(ifs, dummyLine);
    if (debug_read)
      std::cout << "dummyLine 2: " << dummyLine << "\n";
    ...
    for (int j = 0; j < num_of_atoms; ++j)
    {
      ...
      if (!std::count(atom_types.begin(), atom_types.end(), type))
        continue;
      double x, y, z;
      ss >> x >> y >> z;

      if (debug_read)
        std::cout << frame_counter << " : (" << j << " of " << num_of_atoms << ") [" << x << "," << y << "," << z << "]\n";

      auto id = get_atom_id(atom_type_to_id, type);
      if (id == -1)
        continue; // ignore atom
      ...
      for (int area_index = 1; area_index < areas1D.size(); ++area_index)
      {
        ...
        if (areas1D[area_index][0] < tmp_coordinate && tmp_coordinate < areas1D[area_index][1])
        {
          ...
          if (debug_read)
            std::cout << "type " << type << " in area_1D " << area_index << "\n";
